...
export default function SeguridadPrivacidad({ data }) {
  ...
  const cifradoColor = e.includes('CONF') || e.includes('CUMPL') || e.includes('SEGUR')
    ? C.verde
    : e.includes('REVIS') || e.includes('PREC')
      ? C.amarillo
      : C.rojo
      ...
      <div className="p-4 grid grid-cols-2 gap-3">
          ...
          headerRight={
            <div className="relative group shrink-0 ml-auto">
              ...
              <div className="absolute right-0 top-5 z-20 w-64 rounded-lg p-3 text-[10px] leading-relaxed text-slate-300 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-150 font-normal normal-case text-left"
                ...
                <p className="font-bold text-white mb-1.5 border-b pb-1" style={{ borderColor: '#1e3a5f' }}>
                  Glosario · Seguridad de Datos
                </p>
                <div className="space-y-2">
                  <div>
                    <p className="font-semibold text-blue-400">Cifrado AES-256</p>
                    <p className="text-slate-400 text-[9.5px]">
                      Estándar de cifrado militar y gubernamental que protege la información médica en reposo (bases de datos y discos). Su inactividad expone diagnósticos sensibles.
                    </p>
                  </div>
                  <div>
                    <p className="font-semibold text-blue-400">Data Masking (Enmascaramiento)</p>
                    <p className="text-slate-400 text-[9.5px]">
                      Ofuscación dinámica y en tiempo real de datos de identificación (ej. ocultar parte de cédulas o nombres) para usuarios de consulta general sin privilegios.
                    </p>
                  </div>
                  <div>
                    <p className="font-semibold text-blue-400">% Datos Críticos Cifrados</p>
                    <p className="text-slate-400 text-[9.5px]">
                      El avance en la cobertura del cifrado sobre el volumen total de bases de datos críticas del sistema de salud.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <div className="flex flex-col gap-1.5 min-w-0 flex-1">
              ...
              {!cifrado.aes256_activo && (
                <p className="text-[10px] leading-tight" style={{ color: C.rojo }}>
        ...
        <SubBlock icon={ShieldAlert} iconColor={rbacColor} title="Matriz RBAC">
          <div className="flex items-baseline gap-1.5 mt-0.5">
            <span className="text-2xl font-bold tabular-nums" style={{ color: rbacColor }}>
              {rbac.incidentes}
            </span>
        ...
        <SubBlock icon={BarChart3} iconColor={mspiColor} title="MSPI MinTIC">
          ...
          <div className="mt-1">
            ...
            <p className="text-[10px] mt-1.5" style={{ color: C.muted }}>
              {mspiPct}% del objetivo — Modelo de Seguridad y Privacidad
            </p>
          </div>
        </SubBlock>
      </div>
