...
import { Zap, CalendarDays, Banknote } from 'lucide-react'
...
export default function IniciativasPanel() {
  ...
  const barColor = (() => {
    ...
    if (start && end && end > start) {
      ...
      const deviation = ejPct - expected
      if (deviation >= -10) return '#22c55e'
      if (deviation >= -25) return '#f59e0b'
      return '#ef4444'
    }
    return '#6366f1'
  })()
  ...
  return (
    <div className="rounded-xl border overflow-hidden" style={{ background: '#0d1b2e', borderColor: '#1e293b' }}>
      ...
      {/* Body: info left + budget center + grid nav right */}
      <div className="flex justify-between gap-3 p-3">
        ...
        {(() => {
          ...
          const barCol = overBudget ? '#ef4444'
            : deviation >= -10 ? '#22c55e'
              : deviation >= -25 ? '#f59e0b'
                : '#ef4444'
          const amountColor = barCol
          return (
            <div className="rounded-lg border p-3 space-y-2.5 min-w-0" style={{ background: '#111e35', borderColor: '#1e293b', flex: '3.5' }}>
              <div>
                <p className="text-[11px] text-slate-600 uppercase tracking-wide mb-0.5">Presupuesto total</p>
                <p className="text-sm font-bold text-slate-200 tabular-nums">${totalPresupuesto.toLocaleString('es-CO')} <span className="text-[11px] font-normal text-slate-600">COP</span></p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <p className="text-[11px] text-slate-600 uppercase tracking-wide mb-0.5">Ejecutado</p>
                  <p className="text-xs font-bold tabular-nums" style={{ color: amountColor }}>${totalEjecutado.toLocaleString('es-CO')}</p>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <p className="text-sm font-bold tabular-nums" style={{ color: amountColor }}>{pctGlobal}%</p>
                </div>
        ...
        {(() => {
          ...
          sorted.forEach((item, i) => {
            ...
            groups[groups.length - 1].items.push({ item, sortedIdx: i })
          })
